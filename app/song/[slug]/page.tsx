...
const ArtistMVS = ({ slug }: { slug: string }) => {
...
const SimiSong = ({ slug }: { slug: string }) => {
...
const ArtistInfo = ({ slug }: { slug: string }) => {
  const { data: songData } = useSongDetailData(slug);
  const arId = songData?.songs?.[0].ar?.[0].id;
...
const SongComment = ({ slug }: { slug: string }) => {
...
const MusicPlayer = ({ slug }: { slug: string }) => {
  const {
    data: songDetailData,
